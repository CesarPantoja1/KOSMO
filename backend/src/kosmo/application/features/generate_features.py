...
from kosmo.domain.agents.feature_generator.service import generate_features_from_discovery
...
class GenerateFeaturesUseCase:
    ...
    async def execute(self, project_id: ProjectId) -> list[Feature]:
        ...
        generated = await generate_features_from_discovery(
            project_id, self._llm_client, discovery_markdown=discovery_markdown
        )

        new_features = [f for f in generated if _normalize_title(f.title) not in existing_titles]
