...
_DISCOVERY_RESPONSE = json.dumps(
    {
        "vision": "Plataforma que centraliza la gestión de tareas colaborativas con trazabilidad completa y asignación eficiente de responsabilidades entre equipos de trabajo.",
        "problem_space": "Los equipos actualmente gestionan tareas en hojas de cálculo, correos electrónicos y mensajería instantánea, lo que genera desorden, pérdida de información y falta de trazabilidad sobre quién hace qué y cuándo. No existe un mecanismo centralizado que permita crear, asignar, seguir y completar tareas con un flujo claro y auditable.",
        "actors": "Administrador de proyecto: crea y asigna tareas, gestiona el equipo y genera reportes de avance\nUsuario regular: recibe tareas, las ejecuta y reporta avances\nUsuario no autenticado: solo puede ver información pública del proyecto",
        "value_proposition": "Para Administrador de proyecto: centralización total de la gestión de tareas eliminando la dispersión en múltiples canales\nPara Usuario regular: claridad sobre qué hacer, cuándo y para quién, con trazabilidad completa\nPara Organización: reducción del 40% en tiempo perdido por descoordinación y retrabajo",
        "use_cases": "Crear proyecto: el administrador crea un proyecto, define el equipo y configura los permisos de acceso\nAsignar tarea: el administrador crea una tarea con título, descripción y fecha límite, y la asigna a un miembro del equipo\nCompletar tarea: el usuario marca una tarea como completada, adjunta evidencias y el sistema notifica al administrador\nGenerar reporte: el administrador genera un reporte de avance con métricas por usuario y proyecto",
        "core_capabilities": "Gestión de proyectos: creación de proyectos con miembros del equipo y configuración de permisos\nCRUD de tareas: crear, leer, actualizar y eliminar tareas con título, descripción y estado\nAutenticación segura: acceso mediante credenciales con roles de usuario (admin, regular, invitado)\nAsignación de tareas: asignar y reasignar tareas entre miembros del equipo\nDashboard de avance: panel con métricas de productividad por usuario y proyecto\nHistorial de cambios: trazabilidad completa de cada modificación en tareas y proyectos",
        "business_rules": "Solo los administradores pueden crear proyectos y asignar tareas\nUn usuario no puede modificar una tarea que no le ha sido asignada\nLas tareas completadas no pueden ser eliminadas, solo archivadas\nLos reportes solo son accesibles para administradores del proyecto\nCada tarea debe tener un responsable único en todo momento",
        "quality_attributes": "Disponibilidad: el sistema debe estar operativo el 99.5% durante horario laboral\nRendimiento: tiempo de respuesta menor a 500ms para operaciones CRUD\nSeguridad: autenticación con tokens JWT rotativos y roles de acceso\nUsabilidad: interfaz responsive adaptable a dispositivos móviles\nCumplimiento normativo: conformidad con la normativa de protección de datos personales",
        "scope": "Incluido: gestión de tareas con CRUD completo; autenticación JWT con roles de usuario; asignación de tareas entre miembros; dashboard de avance con métricas; historial de cambios con trazabilidad\nExcluido: integración con calendarios externos; notificaciones por email o push; facturación y pagos; integración con sistemas ERP; aplicación móvil nativa\nFuturo potencial: integración con Google Calendar y Outlook; notificaciones en tiempo real por WebSocket; facturación por proyecto con reportes financieros",
    }
)
