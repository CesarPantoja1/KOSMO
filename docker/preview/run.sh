...
#
#   { "<project_id>": { "port": 3001, "workspace": "/workspaces/<project_id>", "url": "http://localhost:3001" } }
#
...
start_project() {
    ...
    workspace="$2"
    port="$3"
    (cd "$workspace" && exec npm run dev -- -H 0.0.0.0 -p "$port") >/dev/null 2>&1 &
    pid=$!
...
write_manifest() {
    ...
    for entry in "$@"; do
        ...
        workspace=$(echo "$entry" | cut -d'|' -f3)
        url="http://localhost:$host_port"
        if [ "$first" -eq 1 ]; then
            manifest="  \"$project_id\": {\"port\": $host_port, \"workspace\": \"$workspace\", \"url\": \"$url\"}"
            first=0
        else
            manifest="$manifest,
  \"$project_id\": {\"port\": $host_port, \"workspace\": \"$workspace\", \"url\": \"$url\"}"
        fi
    done
    ...
    for marker in $(ls "$MARKERS_DIR" 2>/dev/null | sort); do
        ...
        if [ -f "$STATE_DIR/$project_id" ]; then
            ...
            if ! kill -0 "$pid" 2>/dev/null; then
                start_project "$project_id" "$workspace" "$container_port"
        ...
        entries="$entries
$project_id|$host_port|$workspace"
        i=$((i + 1))
    done
