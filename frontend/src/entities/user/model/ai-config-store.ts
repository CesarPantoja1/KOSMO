...
import { DEFAULT_AI_PROVIDER, DEFAULT_AI_MODEL } from './ai-config';
...
export const useAiConfigStore = create<AiConfigState>()((set) => ({
	...
	fetchConfig: async () => {
		...
		try {
			...
			const message = err instanceof Error ? err.message : 'Error al cargar configuración de IA';
	...
	saveConfig: async (data: SaveAIConfigRequest) => {
		...
		try {
			...
			const message = err instanceof Error ? err.message : 'Error al guardar configuración de IA';
			set({ error: message, loading: false });
			throw err;
		}
	},
	...
	deleteConfig: async () => {
		...
		try {
			...
			const message = err instanceof Error ? err.message : 'Error al eliminar configuración de IA';
			set({ error: message, loading: false });
			throw err;
		}
	},
